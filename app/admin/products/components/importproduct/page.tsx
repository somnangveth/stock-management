...
interface MergedImport extends Batch {
  product?: Product;
}
...
export default function ImportProductPage() {
  ...
  const batchQuery = result[1].data as Batch[] | undefined;
  ...
  const pendingImports = useMemo(() => {
    ...
    return batchQuery
      .filter((batch) => batch.import_status === "pending")
      .map((batch) => {
  ...
  // Calculate progress percentage (assuming 14 days is max delivery time)
  const calculateProgress = (daysUntilImport: number) => {
    const maxDays = 14;
    const progress = ((maxDays - daysUntilImport) / maxDays) * 100;
    return Math.max(0, Math.min(100, progress));
  };

  // Get progress color based on days remaining
  const getProgressColor = (daysUntilImport: number) => {
    if (daysUntilImport <= 2) return "bg-red-500";
    if (daysUntilImport <= 5) return "bg-yellow-500";
    return "bg-green-500";
  };
                  ...
                  <div className="mb-4">
                    ...
                    <div className="relative">
                      <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full ${getProgressColor(importItem.days_until_import)} transition-all duration-500 ease-out rounded-full relative overflow-hidden`}
                          style={{ width: `${calculateProgress(importItem.days_until_import)}%` }}
                        >
